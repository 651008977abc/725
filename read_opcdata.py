...
db = client['opcdatabase']
tablefile = db['1210_Axis2_collection']
tabledata = db['1210_Axis2_collection']
...
t=dataf['Time']
timeline=dataf['4# Hours'].astype(float)
# position=dataf['IRB1210_ax2_position'].astype(float)
cycleline=dataf['Cycle'].astype(float)

# 读取mongodb data数据库数据
datad = pd.DataFrame(list(tabledata.find().sort('_id', -1).limit(1)))
benchname=datad['Bench Name'][0]
timetotal=datad['4# Hours'][0]
cycle=datad['Cycle'][0]
date=datad['Date'][0]
...
fig1 = px.line(data_frame=None, x=t, y=timeline)
fig2 = px.line(data_frame=None, x=t, y=cycleline)
st.plotly_chart(fig2)
st.plotly_chart(fig2)
fig1.update_layout(
    xaxis = dict(tickmode='array', # 选择了array 那么就是说要用tickvals 和 ticktext组合了。
                 tickvals = [range(len(t))], # 原标签值
                 ticktext = [t]  # 最终显示的值  (替换的值)
    )
)
fig2.update_layout(
    xaxis = dict(tickmode='array', # 选择了array 那么就是说要用tickvals 和 ticktext组合了。
